...
const map = {
  ...
  render(snakePointsArray, foodPoint, blocksPoint) {
    ...
    const blocksCell = this.cells[`x${blocksPoint.x}_y${blocksPoint.y}`];

    blocksCell.classList.add('blocks');
    this.usedCells.push(blocksCell);
  },
};
...
const snake = {
  ...
  foodCoints() {
    ...
    let foodCountsToken = document.querySelector(".food__counts")
    foodCountsToken.textContent = ""
    foodCountsToken.insertAdjacentHTML("beforeend", `Количество съеденых яблок = ${this.body.length - 1}`)
  },
...
const blocks = {
  ...
  setBlocksCoordinates(point) {
    this.x = point.x;
    this.y = point.y;
  },

  isOnPointBlocks(point) {
    return this.x === point.x && this.y === point.y;
  },
};
...
const game = {
  ...
  reset() {
    ...
    this.blocks.setBlocksCoordinates(this.getRandomFreeCoordinates());
    this.render();
  },
  ...
  getRandomFreeCoordinates() {
    const exclude = [this.food.getCoordinates(), this.blocks.getBlocksCoordinates(), ...this.snake.getBody()];
    // without ... -  [{}, [{}, {}, {}]] => with ... [{}, {}, {}, {}];
    while (true) {
      const rndPoint = {
  ...
  render() {
    this.map.render(this.snake.getBody(), this.food.getCoordinates(), this.blocks.getBlocksCoordinates(), this.snake.foodCoints());
  },
  ...
  tickHandler() {
    ...
    if (this.blocks.isOnPointBlocks(this.snake.getNextStepHeadPoint())) this.finish()
